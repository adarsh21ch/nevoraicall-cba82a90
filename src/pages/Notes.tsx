...
export default function Notes() {
  ...
  const handleDeleteFromList = async (note: Note) => {
    ...
    try { await deleteNote.mutateAsync(note.id); } catch {} finally { setActionNoteId(null); }
  };
  ...
  const handleMoveSave = async () => {
    ...
    try { await updateNote.mutateAsync({ id: moveTarget.id, folder: targetFolder }); setMoveTarget(null); } catch {} finally { setIsMoving(false); setActionNoteId(null); }
  };
      ...
      <div className="sticky top-[105px] z-30 bg-background/95 backdrop-blur-md">
        <div className="container px-4 py-2.5 flex gap-1 overflow-x-auto no-scrollbar">
          {folders.map((f) => (
            ...
            >
              {f === 'All' ? (
                <span className="flex items-center gap-1"><FolderOpen className="h-3 w-3" /> All</span>
              ) : f}
            </button>
          ))}
        </div>
      ...
      {moveTarget && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => setMoveTarget(null)}>
          <div className="bg-card rounded-2xl p-5 w-full max-w-xs space-y-4 shadow-xl animate-in fade-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            ...
            <input value={folderInput} onChange={(e) => setFolderInput(e.target.value)} placeholder="Folder name..." className="w-full h-10 px-3 bg-muted/50 rounded-xl text-sm border border-border/50 outline-none focus:border-accent/50 focus:ring-1 focus:ring-accent/20 transition-all" autoFocus />
            <div className="flex flex-wrap gap-2">
              {folderOptions.map((folder) => (
                <button key={folder} onClick={() => setFolderInput(folder)} className={cn('px-2.5 py-1 rounded-full text-[11px] border transition-colors', folderInput.trim() === folder ? 'bg-accent text-accent-foreground border-accent' : 'bg-muted/30 border-border/60 text-muted-foreground hover:bg-muted/50')}>
                  {folder}
                </button>
              ))}
            </div>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setMoveTarget(null)} className="px-4 py-2 text-xs rounded-lg hover:bg-muted transition-colors">Cancel</button>
              <button onClick={handleMoveSave} disabled={isMoving} className="px-4 py-2 text-xs bg-accent text-accent-foreground rounded-lg font-medium disabled:opacity-60">{isMoving ? 'Saving...' : 'Save'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="fixed bottom-[68px] left-0 right-0 z-40 px-4 pb-3">
        <button
          ...
          className={cn(
            ...
            "shadow-[0_-4px_24px_-6px_rgba(0,0,0,0.06)]",
            "active:scale-[0.98] transition-all duration-200 cursor-text"
          )}
        >
          <div className="w-8 h-8 rounded-xl bg-accent/10 flex items-center justify-center shrink-0">
            {createNote.isPending ? <Loader2 className="h-4 w-4 text-accent animate-spin" /> : <PenLine className="h-4 w-4 text-accent" />}
          </div>
