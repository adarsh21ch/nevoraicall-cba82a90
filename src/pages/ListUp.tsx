  ...
  return <div className="app-layout bg-gradient-to-b from-background via-background to-muted/20">
      <header className="fixed-header z-40 bg-card/80 backdrop-blur-xl border-b border-border/50">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <TeamToggle sharedOwners={sharedOwners} selectedOwnerIds={selectedOwnerIds} onToggleOwner={toggleOwnerSelection} onSelectAll={selectAllOwners} onClear={clearSelection} currentTab="follow_up" />
          </div>
        </div>
      </header>
      ...
      <main ref={containerRef} className="scrollable-content relative pb-20">
        ...
        <div className="container py-3 px-4 space-y-4">
          ...
          {isViewingTeam && <div className="bg-primary/10 border border-primary/30 rounded-xl p-3">
          ...
          <div className="bg-card rounded-xl p-4 border border-border/50 space-y-4">
            ...
            <div className="space-y-2">
              ...
              {stageTags.length === 0 ? <p className="text-xs text-muted-foreground/70">No stage tags</p> : <div className="flex flex-wrap gap-1.5">
                  {stageTags.map(tag => {
            ...
            <div className="space-y-2">
              ...
              {qualityTags.length === 0 ? <p className="text-xs text-muted-foreground/70">No quality tags</p> : <div className="flex flex-wrap gap-1.5">
                  {qualityTags.map(tag => {
                const isSelected = selectedQualities.includes(tag);
                const style = getTagStyle(tag, 'quality', null, isSelected, true);
                return <Badge key={`quality-${tag}`} variant="outline" className="cursor-pointer text-xs transition-all border" style={style} onClick={() => toggleQuality(tag)}>
                        {tag}
                      </Badge>;
              })}
                </div>}
